...
class MainPage(webapp2.RequestHandler):
    ...
    def get(self):
    	guestbook_name = self.request.get('/', DEFAULT_GUESTBOOK_NAME)
...
class Guestbook(webapp2.RequestHandler):
    def post(self):
        ...
        if link.name and link.linkurl:
            link.put()
            self.redirect('/#links')

        else:
            self.response.write("Please input a valid site name and site url! Click <a href=\"http://rare-nectar-94511.appspot.com/#links\">here</a> to be taken back.")

        
        
        query_params = {'/': guestbook_name}
